...
public class TSPDP {
	...
	private int minDistance(int start, HashSet<Integer> thruSet, int returnPoint) {
		...
		if (allDistance[start][indexSet(thruSet)]!=-1) //이미 계산되었다...
			return allDistance[start][indexSet(thruSet)];
		
		if (thruSet.size()==0) { //마지막에 도달했다면 Return 
	...
	// 부분집합을 어떻게 index로 표현하지? => 2진수로 표현하자 => indexSet
	...
	private void findPath(int i, int dist, int endPoint) {
		...
		if (i==endPoint)
			return;
		int indexJ=-1;
		// loop를 돌면서 distance와 맞는 것을 찾아서 indexJ로 반환
		for (int j=0; j< Math.pow(2, numberVertex); j++) {
